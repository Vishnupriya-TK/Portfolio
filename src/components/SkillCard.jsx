...
const SkillCard = ({ title, skills }) => {
  return (
    ...
    >
      ...
      {/* Title */}
      <h3 className="text-lg sm:text-xl md:text-2xl font-semibold text-cyan-300 mb-4 md:mb-6 tracking-wide flex items-center gap-2 font-serif">
        {title}
      </h3>

      {/* Skills */}
      <div className="flex-1 space-y-4 md:space-y-6 font-sans">
        {skills.map((skill, index) => (
          <div key={index} className="space-y-1.5 md:space-y-2">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <p className="text-sm sm:text-base md:text-lg text-gray-200 font-medium tracking-wide">
                {skill.name}
              </p>
            </div>
